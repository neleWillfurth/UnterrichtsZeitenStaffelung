    ...
    for zielpunkt in range(anzahl_schulen_bus):
        for header in range(n_ava[zielpunkt]):
            if startpunkt != zielpunkt:
                while True:
                    travel_time = input(f"Gib die Fahrzeit von Schule {startpunkt + 1} über den Zwischenpunkt {header + 1} zu Schule {zielpunkt + 1} an: ")
...
print(Busankunftszeiten_alle_Schulen_aktuell)
...
for i in range(anzahl_schulen):
    ...
    if Startzeiten_aktuell[i] < 40:
        Startzeiten_Korridor_zulässig.extend([Busankunft_aktuell, Busankunft_aktuell + 5, Busankunft_aktuell + 10, Busankunft_aktuell + 15, Busankunft_aktuell + 20, Busankunft_aktuell + 25, Busankunft_aktuell + 30, Busankunft_aktuell + 35, Busankunft_aktuell + 40])
    elif Startzeiten_aktuell[i] >= 40 and Startzeiten_aktuell[i] < 55:
        Startzeiten_Korridor_zulässig.extend([Busankunft_aktuell - 10, Busankunft_aktuell - 5, Busankunft_aktuell, Busankunft_aktuell + 5, Busankunft_aktuell + 10, Busankunft_aktuell + 15, Busankunft_aktuell + 20])
    else:
        Startzeiten_Korridor_zulässig.extend([Busankunft_aktuell - 10, Busankunft_aktuell - 5, Busankunft_aktuell, Busankunft_aktuell + 5, Busankunft_aktuell + 10, Busankunft_aktuell + 15])
    Busankunftszeiten_alle_schule_zulässig.append(Startzeiten_Korridor_zulässig)

print(Busankunftszeiten_alle_schule_zulässig)
...
for i in range(anzahl_schulen):
    if Haltestellennummern.count(i + 1) == 1:
        Busankunftszeit_Haltestelle.append(Busankunftszeiten_alle_schule_zulässig[i])
    else:
...
Busankunftszeiten_alle_Schulen= [wert for wert, anzahl in zip(Busankunftszeit_Haltestelle, Counter(Haltestellennummern)) for _ in range(anzahl)]
print(Busankunftszeit_Haltestelle)
print(Counter(Haltestellennummern))
print(Busankunftszeiten_alle_Schulen)
...
for Variante in Var:
    ...
    for i in range(anzahl_schulen):
        ...
        s_change = Variante[index-1] + Spaetere_Ankuenfte[i]+Weg_Bus_Schule[i] - Startzeiten_aktuell[i]
