...
def validate_and_submit():
    ...
    try:
        ...
        # Überprüfen, ob alle Elemente in versatz_ankunft_durch_anschlussfahrt_entries durch 5 teilbar sind
        if all(isinstance(x, (int, float)) and x % 5 == 0 for x in[int(entry.get()) for entry in versatz_ankunft_durch_anschlussfahrt_entries]):
            results.append(True)
        else:
            results.append(False)

        # Überprüfen, ob alle Elemente in wegdauer_haltestelle_schule_entries durch 5 teilbar sind
        if all(isinstance(x, (int, float)) and x % 5 == 0 for x in[int(entry.get()) for entry in wegdauer_haltestelle_schule_entries]):
            results.append(True)
        else:
            results.append(False)
        ...
        #del versatz_ankunft_durch_anschlussfahrt_entries[:]
        #del wegdauer_haltestelle_schule_entries[:]
...
def wechsel_tab1_tab2():
    ...
    for i in range(anzahl_Schulen):
        ...
        label_wegdauer_haltestelle_schule_entries = tk.Label(scrollable_tab2.scrollable_frame,text=f"Wie viel Zeit soll bei Schule {i + 1} als Weg von der Bushaltestelle zur Schule eingeplant werden? - Bitte als Vielfaches von 5 angeben")
        label_wegdauer_haltestelle_schule_entries.grid()
...
def s_merkmale_füllen():
    ...
    for haltestellennummern_entries_entry in haltestellennummern_entries:
        haltestellennummern_entries_entry.config(state="disabled")
            ...
            for header in range(int(n_ava[zielpunkt].get())):
                if startpunkt != zielpunkt:
                    ...
                    while True:
                        ...
                        #travel_time = int(entry_Traveltime.get()) + versatz_ankunft_durch_anschlussfahrt_entries[zielpunkt].get()
...
def Berechnung_starten():
    ...
    for i in range(int(anzahl_schulen.get())):
        Busankunft_aktuell = int(aktueller_unterrichtsbeginn_alle_Schulen_entries[i].get()) - int(versatz_ankunft_durch_anschlussfahrt_entries[i].get()) - int(wegdauer_haltestelle_schule_entries[i].get())
    ...
    i = 0
    for i in range(int(anzahl_schulen.get())):
        haltestellennummern_entries_int = [int(entry.get()) for entry in haltestellennummern_entries]
        if haltestellennummern_entries_int.count(i + 1) == 1:
            index = haltestellennummern_entries_int.index(i + 1)
            ...
            for j, busankunftszeit in enumerate(Busankunftszeiten_alle_schule_zulässig):
                if haltestellennummern_entries_int[j] == i + 1:
    ...
    print(mindestbuseinsparung_hohe_einsparung)
    ...
    for Durchläufe in range(max_reuses - mindestbuseinsparung_hohe_einsparung + 1):
        ...
        print("max_reuses-Busse hohes Einsparpotenzial", max_reuses - mindestbuseinsparung_hohe_einsparung)
    ...
    label_hohe_Einsparung = ttk.Label(scrollable_tab6.scrollable_frame,text=f"Ein hohes Einsparpotenzial wird ab{mindestbuseinsparung_hohe_einsparung} erreicht")
    ...
    for Durchläufe in range(max_reuses - mindestbuseinsparung_hohe_einsparung + 1):
        ...
        for q in indices_of_values[Durchläufe]:
            ...
            for i, inner_list in enumerate(schoolstart_combination):
                ...
                #anzahl = int(haltestellennummern_entries.get()).count(nummer)
                #[int(haltestellennummern_entriesEntery.get()) for haltestellennummern_entriesEntery in haltestellennummern_entries]
                #item_counts = Counter([haltestellennummern_entriesEntery.get() for haltestellennummern_entriesEntery in haltestellennummern_entries])
                ...
                for schule in range(int(anzahl_schulen.get())):
                    haltestelle = haltestellennummern_entries_int[schule] -1
                    ankunftszeit = inner_list[haltestelle]
                    Busankunft_Liste_Alle_Schulen.append(ankunftszeit)
                    Unterrichtsbeginn_Aktuelle_Schule = int(wegdauer_haltestelle_schule_entries[schule].get())+int(versatz_ankunft_durch_anschlussfahrt_entries[schule].get())+ankunftszeit
            ...
            #   Unterrischtsbeginn_1_Schule = int(wegdauer_haltestelle_schule_entries[i].get())+int(versatz_ankunft_durch_anschlussfahrt_entries[i].get())+int(Busankunft_Liste_Alle_Schulen[i]))
...
def Akzeptanzbewertung_anzeigen():
    ...
    for Variante in Var:
        ...
        anzahl_schulen=int(entry_anzahl_schulen.get())
        for i in range(anzahl_schulen):
            index = int(haltestellennummern_entries[i].get())
            s_change = Variante[index - 1] + int(versatz_ankunft_durch_anschlussfahrt_entries[i].get()) + int(wegdauer_haltestelle_schule_entries[i].get()) - int(aktueller_unterrichtsbeginn_alle_Schulen_entries[i].get())
